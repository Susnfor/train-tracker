...
export default function ServicesStatus(){
    ...
    async function fetchTrainData() {
        try {
        const res = await fetch('https://api.tfl.gov.uk/Line/Mode/tube,dlr,overground,elizabeth-line,tram/Disruption?app_key=d7ea9b1b04bc415c9a83b3636e9b9213')
            ...
            <div>
               {
                data && data.map((item: any, index: number) => {
                    return (
                        <div>
                        <h1 key={item.closureText}>{item.closureText}</h1>
                            <h2 key={item.type}>{item.description}</h2>
